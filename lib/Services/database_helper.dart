...
class DatabaseHelper {
  ...
  Future<void> addContent(Map<String, dynamic> data) async => await insert(tableContents, data);
  Future<void> updateContent(String id, Map<String, dynamic> data) async => await update(tableContents, data, id);
  Future<void> deleteContent(String id) async => await delete(tableContents, id);
  ...
  Future<void> toggleLike(String id, bool add) async {
    final db = await database;
    await db.rawUpdate('UPDATE $tableContents SET likes = likes ${add ? '+' : '-'} 1 WHERE id = ?', [id]);
  }
